...
from tc2.strategy.strategies.swing1 import swing_constants
from tc2.util.TimeInterval import TimeInterval
from tc2.util.date_util import TIME_FORMAT


class SwingStrategy(AbstractStrategy):
    """
    SwingStrategy looks at daily aggregate data to find a symbol that is likely to profit within several days' time.
    """

    @classmethod
    def times_active(cls) -> TimeInterval:
        return TimeInterval(logfeed=None,
                            start1=time(hour=10, minute=0),
                            end1=time(hour=15, minute=0))

    @classmethod
    def pass_fail_models(cls) -> List[AnalysisModelType]:
        return [
            AnalysisModelType.HIGH_96_PCT,
            AnalysisModelType.VOLUME_50
            # TODO Add MACDModel
            # TODO Add RSIModel
            # TODO Add Volume50Model
        ]
    ...
    def buy_logic(self) -> None:
        if self.insufficient_funds():
            ...
            self.info_process('SwingStrategy ending buy logic. Insufficient funds')
        ...
        if not self.printed_buy_start:
            if len(self.recent_candles) == 0:
                self.info_process('SwingStrategy waiting for price data')
                return
            latest_price = self.recent_candles[-1].low
            self.buy_price = latest_price + max(0.01, latest_price * (swing_constants.RAISE_BUY_PERCENT / 100))
            self.qty = self.get_qty_at_price(self.buy_price)
            if self.acct.place_limit_buy(self.get_symbols()[0], self.buy_price, self.qty):
                self.info_process(
                    'Starting SwingStrategy buy logic on ' + str(self.qty) + ' ' + self.get_symbols()[
                        0] + ' for $' + str(
                        self.buy_price))
                self.printed_buy_start = True
            else:
                self.warn_process('SwingStrategy couldn\'t place buy order')
            return

        # Wait for the price to dip and trigger our buy order

        if self.climbed_too_much():
            self.info_process('SwingStrategy ending buy logic after price climbed too high')
            self.stop_running(sell_price=None)
            return

        if self.waited_too_long_for_buy():
            self.info_process('SwingStrategy ending buy logic after waiting too long for a buy order to go through')
            self.stop_running(sell_price=None)
            return
    ...
    def sell_logic(self) -> None:
        ...
        if self.not_bought_yet():
            self.warn_process("SwingStrategy can't sell while new positions not detected")
            return
        ...
        if not self.printed_sell_start:
            ...
            self.stop_price = self.buy_price * (1 - (swing_constants.MAX_STOP_PCT / 100))
            self.sell_target = self.stop_price
            self.info_process(
                'SwingStrategy initial stop order: {0} {1} for ${2}'.format(
        ...
        if self.symbol_was_sold():
            self.info_process(
                'SwingStrategy sold {0} {1} for {2}% profit after {3} seconds'.format(
                    self.qty,
                    self.get_symbols()[0],
        ...
        # A) [y < -0.8%] Look ahead to cut losses (i.e. place a stop order) when the price dips low
        if current_price < self.buy_price * (1 - (0.8 / 100)):
            self.sell_target = self.stop_price
            if self.too_close_to_last_offer():
                return
            self.acct.place_stop_order(self.get_symbols()[0], self.sell_target, self.qty)
            self.info_process(
                'SwingStrategy placing stop order at ${0}'.format("%.2f" % self.sell_target))
            return

        # TODO Place a non-greedy offer to exit the positions after it hasn't turned profitable within several days

        # TODO Place limit sell order just below the resistance line

    def insufficient_funds(self) -> bool:
        return self.acct.withdrawable_balance < swing_constants.MIN_PURCHASE
    ...
    def climbed_too_much(self) -> bool:
        return self.recent_candles[-1].close >= self.buy_price * (
                1.0 + (swing_constants.MAX_PCT_INCREASE_BEFORE_BUY / 100.0))

    def waited_too_long_for_buy(self) -> bool:
        return self.time().now() >= self.run_info.start_time + timedelta(seconds=swing_constants.MAX_BUY_TIME)
    ...
    def merge_candle(self, candle: Candle) -> None:
        ...
        if len(self.recent_candles) <= 60 * swing_constants.BREAKOUT_SETUP_MINS:
            return
        oldest_candle = None
