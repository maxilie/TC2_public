...
class CycleStrategy(AbstractStrategy):
    ...
    def buy_logic(self) -> None:
        """self.info_process('CycleStrategy running buy logic at {0}:{1}:{2}'
        ...
        if self.insufficient_funds():
            ...
            self.info_process('CycleStrategy ending buy logic. Insufficient funds')
        ...
        if not self.printed_buy_start:
            if len(self.candles_5) == 0:
                self.info_process('CycleStrategy waiting for price data')
                return
            ...
            if self.acct.place_limit_buy(self.get_symbols()[0], self.buy_price, self.qty):
                self.info_process(
                    'Starting CycleStrategy buy logic on ' + str(self.qty) + ' ' + self.get_symbols()[
                        0] + ' for $' + str(
                        self.buy_price))
                self.printed_buy_start = True
            else:
                self.warn_process('CycleStrategy couldn\'t place order')
            return

        # Wait for the price to dip and trigger our buy order

        if self.climbed_too_much():
            self.info_process('CycleStrategy ending buy logic after price climbed too high')
            self.stop_running(sell_price=None)
            return

        if self.waited_too_long_for_dip():
            self.info_process('CycleStrategy ending buy logic after waiting too long for a price dip')
            self.stop_running(sell_price=None)
            return

    def sell_logic(self) -> None:
        # self.info_process('CycleStrategy running sell logic at {0}'.format(self.live_time_env.now()))
        # Don't sell until the brokerage confirms our new positions
        if self.not_bought_yet():
            self.warn_process("CycleStrategy can't sell while new positions not detected")
            return
        ...
        if not self.printed_sell_start:
            ...
            self.info_process(
                'CycleStrategy initial stop order: {0} {1} for ${2}'.format(
        ...
        if self.symbol_was_sold():
            self.info_process(
                'CycleStrategy sold {0} {1} for {2}% profit after {3} seconds'.format(
                    self.qty,
                    self.get_symbols()[0],
        ...
        if self.market_closes_soon():
            ...
            self.info_process('CycleStrategy placing non-greedy offer since markets close soon: ${0}'
        ...
        if current_price < self.buy_price * (1 - (0.3 / 100)):
            ...
            self.acct.place_stop_order(self.get_symbols()[0], self.sell_target, self.qty)
            self.info_process(
                'CycleStrategy placing stop order at ${0}'.format("%.2f" % self.sell_target))
            return
        ...
        if current_price < self.buy_price * (1 + (0.12 / 100)):
            ...
            self.info_process('CycleStrategy setting baseline sell order at ${0}'.format("%.2f" % self.sell_target))
    ...
    def start_next_cycle(self, current_price: float, safety_sell_point: float, greedy_sell_point: float) -> None:
        ...
        if self.too_close_to_last_offer():
            self.info_process('CycleStrategy offer too close to last one. Not updating')
            return
        self.acct.place_limit_sell(self.get_symbols()[0], self.sell_target, self.qty)
        self.info_process('CycleStrategy Cycle #{0} price: ${1}  {2}'
