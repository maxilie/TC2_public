...
@Component({
    selector: 'app-price-graph-content',
    templateUrl: './price-graph-content.component.html',
    styleUrls: ['./price-graph-content.component.less'],
})

export class PriceGraphContentComponent implements OnInit {
    selectedSymbol = 'TXN';
    ...
    public refreshVisual(firstCall: boolean) {
        ...
        const priceGraph = this;

        function peek() {
            if (!priceGraph.updating) {
                return;
            }

            const oldUpdateTime = priceGraph.lastUpdated;
            ...
            function checkTimeLastUpdated(data) {
                ...
                if (priceGraph.refreshChecks >= 20) {
                    // Stop checking after 20 attempts
                    priceGraph.updating = false;
                    priceGraph.refreshChecks = 0;
                } else if (updateTime === null || updateTime <= oldUpdateTime) {
                    // Check again in 3 seconds
                    priceGraph.refreshChecks += 1;
                } else if (priceGraph.refreshChecks < 20) {
                    // Update visual display
                    priceGraph.applyVisualData(data);
                    priceGraph.updating = false;
                    priceGraph.refreshChecks = 0;
                }
            }

            priceGraph.api.getVisualData(checkTimeLastUpdated, {
                'visual_type': 'price_graph',
                'symbol': priceGraph.selectedSymbol
            })
        }
    ...
    public applyVisualData(data) {
        ...
        this.createPriceGraph();
    }

    public createPriceGraph() {
